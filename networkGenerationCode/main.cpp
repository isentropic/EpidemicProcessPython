...
int power_dist_determenistic(double power_law_exponent, int k_max, int k_min, int N, int *seq){
	int seq_len = k_max - k_min + 1;
	double Pk_normalization_constant = (1.0 - power_law_exponent)/(std::pow(k_max, 1.0-power_law_exponent) - std::pow(k_min, 1.0-power_law_exponent) );
	double interval_prob[seq_len];
	//std::cout<<Pk_normalization_constant<<" normconstant \n";


	for(int i = seq_len-1; i>=0; i--){
		interval_prob[i] = Pk_normalization_constant*std::pow(k_min+i, -power_law_exponent);
	}
...
std::vector<int> generate_communites(int total_nodes){
	...
	calculated_N = power_dist_determenistic(beta_power, s_max, s_min, trial_N, pointer_to_holder);
	int index = 0;
	if (calculated_N>target_N){
		std::cout<<"Wrong trial_N was chosen\n";
	}
	while(calculated_N<target_N){
		calculated_N = power_dist_determenistic(beta_power, s_max, s_min, trial_N+index,pointer_to_holder);
		index++;
	...
	calculated_N = power_dist_determenistic(beta_power, s_max, s_min, trial_N+index-2,pointer_to_holder);
...
int main(int argc, const char * argv[]) {
	...
	if (sum_vector(communities) > N){
		...
		//return 0;
		communities[no_of_communites-1] = communities[no_of_communites-1] - sum_vector(communities) + N;
	}
	...
	for(int i = 0; i<no_of_communites;i++){
		...
		for(int j =0; j<links_per_com; j++){
			...
			if (nodes[secondnode]->empty()){
				std::cout<<"Alert, inconsistency at removing random nodes:2nd node is empty 1\n";
				throw 1;
			}
			...
			if (nodes[firstnode]->contains(secondnode+1) && nodes[secondnode]->contains(firstnode+1)){
	...
	while(links_removed>0 && to_be_recconected_nodes.size()>0){
		...
		//std::cout<<"test\t";
		while( nodes.at( to_be_recconected_nodes.at(firstnode) )->community_number == nodes.at(to_be_recconected_nodes.at(secondnode))->community_number ){
		//while(true){
			//std::cout<<"test3\t";
			secondnode = (uni(engine)%to_be_recconected_nodes.size());
			safety_while1 ++;
		...
		if(firstnode>secondnode){
			to_be_recconected_nodes.erase(to_be_recconected_nodes.begin() + firstnode);
			to_be_recconected_nodes.erase(to_be_recconected_nodes.begin() + secondnode);
		}
		else{
			to_be_recconected_nodes.erase(to_be_recconected_nodes.begin() + secondnode);
			to_be_recconected_nodes.erase(to_be_recconected_nodes.begin() + firstnode);
	...
	if(too_restricted){
		...
		std::cout<<"emergency actions taken place: "<<resrsize<<" nodes were connected randomly\n";
	}
	...
	for (int i=0; i<N; i++){
		degree_distr<<i<<"\t"<<
			float(nodes[i]->connectivity_global)/(nodes[i]->connectivity)<<"\t"<<nodes[i]->connectivity<<"\t"<<nodes[i]->connectivity_local<<"\t"<<nodes[i]->connectivity_global<<"\t\n";
	...
	for(int i =0; i<no_of_communites; i++){
		...
		for(int j = 0; j<communities[i];j++){
			average_mu_community+=  float(nodes[index_communites[i]+j]->connectivity_global)/(nodes[index_communites[i]+j]->connectivity);
		}
