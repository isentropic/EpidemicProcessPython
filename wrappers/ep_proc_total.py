...
additionalcomment = '_ER_4'
samplesizes = ['4e3', '8e3', '16e3', '32e3']
samplename = ''
...
os.system('mkdir -p %s/results/randPunited%s/P_infVsLambda' % (fullpath, samplename))
os.system('mkdir -p %s/results/randPunited%s/rvslambda' % (fullpath, samplename))
...
def get_total_value(proc_id, lambdaval, T):
    os.system("%s/total_epid_proc %s %f %f" %(fullpath,proc_id,lambdaval,T))
...
if __name__ == '__main__':
    ...
    os.system("g++  -std=c++11 -I %s %s/Linked_list.cpp %s/epid_process_total.cpp -o %s/total_epid_proc" %(fullpath,fullpath,fullpath,fullpath))
    ...
    for asize in samplesizes:
        ...
        my1stfile = open('%s/results/randPunited%s/P_infVsLambda/P_inf_raw%s.txt'%(fullpath,samplename,asize),'w')
        my2ndfile = open('%s/results/randPunited%s/rvslambda/raw_results%s.txt'%(fullpath,samplename,asize),'w')
